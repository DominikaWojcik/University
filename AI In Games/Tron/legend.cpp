...
struct MCTNode
{
	...
	vector<double> childTotalRewards;
	...
	int getBestChild()
	{
		...
		{
			double UCT = childTotalRewards[i] / timesChildPlayed[i] + 
...
private:
	...
	void initializeChildren()
	{
		...
		childTotalRewards.resize(possibleActions.size(), 0.);
	}
};
...
vector<double> voronoiRolloutPolicy(MCTNode& node)
{
	...
	vector<double> rewards = vector<double>(state.players, 0.);
	...
	if(!state.isTerminal())
	{
		...
		{
			total += voronoiSizes[i];
			rewards[i] = voronoiSizes[i];
		}
		if(total == 0) total = 1.;
	...
	{
		rewards[state.currentPlayer] = 1.;
	}

	return rewards;
}

void backup(MCTNode& node, vector<double>& rewards)
{
	...
	while(currentNode != nullptr)
	{
		...
		currentNode->childTotalRewards[childId] += rewards[currentNode->player];
...
action MCTS(Stopwatch<>& sw)
{
	...
	do
	{
		...
		vector<double> rewards = voronoiRolloutPolicy(chosenNode);
		backup(chosenNode, rewards);
